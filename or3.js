...
//  function b_x(t) { t = t % 8; return t<2 ? 0 : t<3 ? 0.5-0.5*Math.cos(Math.PI*t) : t<6 ? 1 : t<7 ? 0.5+0.5*Math.cos(Math.PI*t) : 0; }
  ...
  var a_bar = new Bar({ origin:{x:100,y:center - input_spacing}, length:250, angle:0, stroke:color_a});
  var b_bar = new Bar({ origin:{x:100,y:center}, length:250, angle:0, stroke:color_b});
  var c_bar = new Bar({ origin:{x:100,y:center + input_spacing}, length:250, angle:0, stroke:color_c});  
  ...
  var o_bar = new Bar({ length:250, stroke:color_o});
  ...
  processing.draw = function() {
    ...
    ab_link.joinEnds(ba_link, true);
    bc_link.joinEnds(cb_link, true);
        
    //Move the first output links
    abo_link.origin = ab_link.end();
    bco_link.origin = bc_link.end();
    abo_link.setEndY(center - input_spacing/2, true);
    bco_link.setEndY(center + input_spacing/2, true);
    
    //Move the second input links
    m1_link.origin = abo_link.end();
    m2_link.origin = bco_link.end();
    m1_link.joinEnds(m2_link, true);
    ...
    processing.text("3-Input OR GATE Simulation", 40,30);
